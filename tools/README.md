...
本目录存放**可重复执行的辅助脚本**。技能主流程以 `SKILL.md` 与 `prompts/` 为准；本目录侧重格式转换等可执行工具。当前 fork 的 Step 5 以 Zhihuiya/Patsnap 为主，国知局脚本作为官方公布公告补充或 fallback。

## 国知局公布公告检索（epub.cnipa.gov.cn，Step 5 fallback / 补充）

| 脚本 | 作用 |
|------|------|
| **`cnipa_epub_search.py`** | **（Step 5 fallback / 补充）** 一步：拉取 + 解析，**不写结果页 HTML 落盘**；**Agent 须按 `prior_art_search.md` 分多次调用、每轮一词并自行合并 JSON**；脚本在**单次命令多词**时也会进程内循环检索并合并（人工/本地便利）；**stdout 仅一行** `EPUB_HITS_JSON:`；stderr 上 `EPUB_*` 为 **ASCII**；UTF-8 / PowerShell 见 **INSTALL.md**。 |
...
Zhihuiya 不可用、需官方公布公告佐证，或国知局抓取失败/解析无命中时，Agent 按 **`prompts/prior_art_search.md`** 再补充 **WebSearch**（如 Google 学术 / Google Patents）。
